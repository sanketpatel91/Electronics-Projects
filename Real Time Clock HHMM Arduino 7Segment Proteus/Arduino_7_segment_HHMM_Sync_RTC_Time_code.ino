 ...
 * -  4Digit 7 segment displaly(Common Cathod) pins a to g are coneected to Arduino Digital pin 1 to 6.
...
void loop() {
 ...
 //Display Selection to increse digits
  ...
  //rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));
    rtc.adjust(DateTime(2022, 02, 17, (hh1cnt * 10) + hh2cnt , (mm1cnt * 10) + mm2cnt, 0)); // (YYYY,MM,DD,HH,MM,SS)
//    delay(100);
    disp_Off();
    delay(50);
  ...
  if (Update_Mode == 0){
    ...
    //  Serial.print(now.hour(), DEC);  //  Serial.print(now.minute(), DEC);
    Hrs = now.hour();
    Min = now.minute();
